...
for STEP = 1:STEP_STEPS_NUM
    ...
    for i = 1:ASSEMBLY_ELEMENTS_NUM
        ...
        for ig = 1:3
            ...
            % COMPUTE STRESS USING NEO-HOOKEAN CONSTITUTIVE MODEL
            % Calculate deformation gradient components
            F11 = F_GRAD_11;
            F33 = F_GRAD_33;
            
            % For axisymmetric shell (assuming F22 = F33 for hoop symmetry)
            F22 = F33;
            
            % Calculate invariants
            J = F11 * F22 * F33;  % Jacobian (volume ratio)
            I1 = F11^2 + F22^2 + F33^2;  % First invariant of right Cauchy-Green tensor
            
            % Material parameters
            lambda = MATERIAL_BULK_MODULUS - (2/3) * MATERIAL_SHEAR_MODULUS;  % Lamé parameter
            mu = MATERIAL_SHEAR_MODULUS;
            
            if ig < 5
                % Neo-Hookean Cauchy stress tensor
                % σ = (μ/J)(b - I) + (λ/J)(ln(J))I
                % where b is the left Cauchy-Green tensor (b = F*F^T)
                
                % Left Cauchy-Green tensor components (for diagonal F)
                b11 = F11^2;
                b22 = F22^2;
                b33 = F33^2;
                
                % Cauchy stress components
                SIGMA_11 = (mu/J) * (b11 - 1) + (lambda/J) * log(J);
                SIGMA_22 = (mu/J) * (b22 - 1) + (lambda/J) * log(J);
                SIGMA_33 = (mu/J) * (b33 - 1) + (lambda/J) * log(J);
                
                % Shear stresses (zero for axisymmetric stretching)
                SIGMA_12 = 0;
                SIGMA_21 = 0;
            end
    ...
    if STEP_TIME >= STEP_LOAD(1) && STEP_TIME < STEP_LOAD(2)
        FORCE_EXTERNAL_GLOBAL(ASSEMBLY_NODES_NUM + 1) = FORCE_EXTERNAL_VALUE*STEP_TIME/STEP_LOAD(2);
